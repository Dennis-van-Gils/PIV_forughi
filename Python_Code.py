...
def fixer(
    vecx, vecy, vec, rij, r_limit, i_fix
    ...
    for j in range(1, vec.shape[1] - 1):
        for i in range(1, vec.shape[0] - 1):
            ...
            res_s_x = np.abs(vecx[i, j] - np.median(neigh_x)) / (
                np.median(np.abs(res_x)) + 0.1
            )  # Normalized Residual (Epsilon=0.1)
            res_s_y = np.abs(vecy[i, j] - np.median(neigh_y)) / (
                np.median(np.abs(res_y)) + 0.1
            )

            fluc[i, j] = np.sqrt(
                res_s_x * res_s_x + res_s_y * res_s_y
    ...
    for ii in range(i_fix):  # Correction Cycle for patches of bad data
            ...
            for i in range(1, vec.shape[0] - 1):
                if fluc[i, j] > 2.0 or (
                    ...
                    vec_diff += (
                        vec[i, j]
                        - np.sqrt(
                            vecx[i, j] * vecx[i, j] + vecy[i, j] * vecy[i, j]
                        )
                    ) ** 2.0
                    vec[i, j] = np.sqrt(
                        vecx[i, j] * vecx[i, j] + vecy[i, j] * vecy[i, j]
        ...
        if i_disorder == 0 or vec.mean() == 0.0:
            break  # No need for correction
        correction_residual = vec_diff / (i_disorder * np.abs(vec.mean()))
        if correction_residual < 1.0e-20:
            break  # Converged!
    if ii == i_fix - 1:
        print("Maximum correction iteration was reached!")
...
def subpix(R, axis):  # Subpixle resolution (parabolic-Gaussian fit)
    ...
    try:  # Out of bound at the edges:
        if axis == "y":  # For vecy
            ...
            r_w = R[R_x, R_y - 1]
        if (
            r_e > 0.0 and r_w > 0.0 and r > 0.0
        ):  # Gaussian if possible (resolves pick locking)
            r_e = np.log(r_e)
            r_w = np.log(r_w)
            r = np.log(r)
        if (r_e + r_w - 2 * r) != 0.0:
            if (
                np.abs((r_w - r_e) / (2.0 * (r_e + r_w - 2 * r))) < 1.0
                and np.abs(r_e + 1) > 0.01
                and np.abs(r_w + 1) > 0.01
            ):
...
img_1 = (np.flip(cv2.imread("a1.png", 0), 0)).astype(
    "float32"
)  # Read Grayscale
...
cores = (
    -1
)  # Number of parallel jobs: 1 = no parallel processing ; 2 and above = number of parallel processes ; -1 = maximum
...
margin = int((sw - iw) / 2)
im = int(
    2 * np.floor((ia - 1 - iw) / (iw - 1))
)  # Number of I.W.s in x direction
jm = int(
    2 * np.floor((ja - 1 - iw) / (iw - 1))
)  # Number of I.W.s in y direction
...
reconst = Parallel(n_jobs=cores)(
    delayed(jay_walker)(j) for j in tqdm(range(jm))
)
for reoncs_row in reconst:
    (
        ...
        vecy[:, reoncs_row[0]],
        rij[:, reoncs_row[0]],
    ) = (reoncs_row[1], reoncs_row[2], reoncs_row[3], reoncs_row[4])

# %% Corrections:
vecx, vecy, vec, i_disorder, i_cor_done = fixer(
    vecx, vecy, vec, rij, r_limit, i_fix
)

# %% Applying the scales:
X, Y = np.meshgrid(
    np.arange(0.5 * iw, 0.5 * iw * (jm + 1), 0.5 * iw),
...
plt.contourf(
    ...
    rij,
    cmap="jet",
    levels=np.arange(rij.min(), min(rij.max() + 0.1, 1.0), 0.01),
)
